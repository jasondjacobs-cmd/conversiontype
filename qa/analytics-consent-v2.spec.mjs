...
const collection=url=>url.includes('google-analytics.com')&&(/\/g\/collect|\/collect/.test(url));
...
test('GA4 requests require opt-in and stop after revocation',async({browser,baseURL})=>{
 ...
 page.on('requestfailed',request=>{if(google.test(request.url()))failures.push(`${request.url()}: ${request.failure()?.errorText}`)});
 ...
 await expect.poll(()=>requests.some(url=>url.includes('googletagmanager.com/gtag/js?id=G-SHR0ETX628')),{timeout:15000,message:'Correct GA4 tag should load after opt-in'}).toBe(true);
 await expect.poll(()=>requests.some(collection),{timeout:20000,message:'GA4 collection should occur after opt-in; failed requests: '+failures.join(' | ')}).toBe(true);
