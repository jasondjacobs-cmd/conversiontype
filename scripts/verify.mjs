...
const html=await readFile('index.html','utf8'),app=await readFile('app.js','utf8'),styles=await readFile('styles.css','utf8'),ratio=await readFile('ratio.js','utf8'),percentage=await readFile('percentage.js','utf8'),fraction=await readFile('fraction.js','utf8'),map=await readFile('sitemap.xml','utf8'),manifest=JSON.parse(await readFile('site.webmanifest','utf8'));
...
for(const p of pages){
 ...
 checks.push(
  ...
  [body.includes('class="content"'),p+' crawlable definition context']
 );
...
checks.push(
 [!/<loc>[^<]*\?/.test(map),'sitemap excludes query states'],
 [app.includes('Definition Bubbles v1'),'definition bubble engine'],
 [app.includes("document.querySelectorAll('.content p,.content li')"),'definitions limited to explanatory content'],
 [app.includes("setAttribute('aria-expanded','false')"),'definition trigger expanded state'],
 [app.includes("event.key==='Escape'"),'definition escape dismissal'],
 [app.includes("setAttribute('role','dialog')"),'definition dialog semantics'],
 [app.includes('textContent=def.text'),'definition text inserted safely'],
 [styles.includes('.definition-term:focus-visible'),'definition keyboard focus style'],
 [styles.includes('calc(100vw - 24px)'),'definition mobile viewport constraint'],
 [styles.includes('.definition-close'),'definition close control style'],
 [!app.includes('window.open('),'definitions do not use browser popups']
);
for(const[passed,name]of checks)if(!passed)throw new Error('Release gate failed: '+name);
console.log('Passed '+checks.length+' platform, SEO, and definition-bubble release checks.');
